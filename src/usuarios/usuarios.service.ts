...
export class UsuariosService {
  constructor(
    ...
    @InjectRepository(Provincia) // Inyectamos Provincia
    private readonly provinciaRepository: Repository<Provincia>,

    @InjectRepository(Rol) // Inyectamos Provincia
  ...
  async create(usuario: CreateUsuarioDto): Promise<ResponseDTO> {
    ...
    console.log("que esta llegando en provincia:")
    console.log(usuario.id_provincia);
  ...
  async update(
    ...
    const datosUpdate:any={...modificaciones}
    if (modificaciones.id_provincia){
      datosUpdate.provincia={id:modificaciones.id_provincia};
      delete datosUpdate.id_provincia;
    }
    if (modificaciones.id_rol){
      datosUpdate.rol={id_rol:modificaciones.id_rol};
      delete datosUpdate.id_rol;
    }

    const res = await this.usuarioRepository.update(dni, datosUpdate);
    if (!res.affected)
      throw new NotFoundException(`No se encontro DNI con ese numero.`);
    return {
      statusCode: HttpStatus.OK,
